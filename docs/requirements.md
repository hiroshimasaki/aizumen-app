...
| **AI/OCR** | Google Gemini API | テナントごとにプロンプトカスタマイズ可 |
| **決済** | Stripe | サブスクリプション + 従量課金（AIクレジット） |
| **ホスティング** | Vercel（FE）+ Railway（API） | 推奨。詳細は§2.3 |
...
| 15 | 見積印刷 | そのまま流用 |
| 16 | スマート見積 | AIクレジット消費 |
| 17 | バックアップ/復元 | DB自動バックアップ（Supabase管理）に変更 |
| 18 | 会社マスタ | DB管理に変更。テナント分離 |
...
| プラン | ユーザー数 | 月額（税別） | 単価/ユーザー | 年額（税別） | 年額月あたり |
|-------|-----------|------------|-------------|------------|------------|
| **Small** | 5ユーザーまで | ¥10,000 | ¥2,000 | ¥108,000 | ¥9,000 (10%OFF) |
| **Medium** | 20ユーザーまで | ¥30,000 | ¥1,500 | ¥324,000 | ¥27,000 (10%OFF) |
| **Large** | 50ユーザーまで | ¥50,000 | ¥1,000 | ¥540,000 | ¥45,000 (10%OFF) |
