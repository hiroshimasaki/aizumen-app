...
| **AI/OCR** | Google Gemini 1.5 Flash / pgvector | 注文書OCR、類似図面検索（ハイブリッドpHash + ベクトル）、価格推定 |
...
| 14 | ダッシュボード | そのまま流用 |
| 15 | 見積印刷 | そのまま流用 |
| 16 | 類似図面検索 | pHash(高速一致) + ベクトル検索 + Geminiリランキング のハイブリッド方式 |
...
| 消費タイミング | OCR自動登録、スマート見積、AI類似検索（ハイブリッド精査）利用時。※pHash完全一致時は消費なし。 |
